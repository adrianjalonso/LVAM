...
import type { Perfume } from "./types/Perfume.ts";
import type { Carrinho } from "./types/FavoritesAndCarrinho.ts";
  ...
  async function obterTotais() {
    ...
    if (data) {
      ...
      setTotalMasculino(
        data.filter(
          (perfume) =>
            perfume.genero === "masculino" && carrinho.includes(perfume.id)
        ).length
      );
      setTotalFeminino(
        data.filter(
          (perfume) =>
            perfume.genero === "feminino" && carrinho.includes(perfume.id)
        ).length
      );
      setTotalKids(
        data.filter(
          (perfume) =>
            perfume.genero === "kids" && carrinho.includes(perfume.id)
        ).length
      );
    }
  }
    ...
    <div className="grid md:grid-cols-[repeat(3,minmax(200px,1fr))] lg:grid-cols-[repeat(4,minmax(200px,1fr))] grid-cols-[repeat(2,minmax(150px,1fr))] gap-4  w-full md:w-5/6 lg:w-4/5 pt-20 pb-4 ">
      ...
      <div className="grid md:grid-cols-[repeat(3,minmax(200px,1fr))] lg:grid-cols-[repeat(4,minmax(200px,1fr))] grid-cols-[repeat(2,minmax(150px,1fr))] gap-4  w-full md:w-5/6 lg:w-4/5 pb-4 ">
        {products.map((perfume) => {
          ...
          const textCarrinhoButton = isInCarrinho
            ? "Tirar do carrinho"
